...
def verify_email(token: str = Query(...), controller: UserController = Depends(get_user_controller)):
    try:
...
def password_reset_request(email: str = Body(...), controller: AuthController = Depends(get_auth_controller)):
    try:
        controller.request_password_reset(email)
        return {"msg": "If your email is registered, you will receive reset instructions."}
    except ValueError:
        # Return same message to avoid email enumeration
        return {"msg": "If your email is registered, you will receive reset instructions."}

@router.post("/password-reset")
def password_reset(token: str = Body(...), new_password: str = Body(...), controller: AuthController = Depends(get_auth_controller)):
    try:
